...
const { checkSentDetails, checkPassengerExists, createLoginSecret } = require('../middleware/passenger')
...
router.post('/login', checkSentDetails, checkPassengerExists, generate_TOTP, async (req, res) => {
    ...
    try {
      ...
      res.status(200).send({ message:"Please enter code sent to your phone", totp: totp})
    }
    catch(error) {
